...
def make_prediction(input_data, model, feature_names, original_df):
    """Make prediction for individual input"""
    try:
            ...
            if feature not in input_df.columns:
                ...
                if feature in original_df.columns:
                    if original_df[feature].dtype in ['int64', 'float64']:
                        input_df[feature] = original_df[feature].median()
                    else:
                        input_df[feature] = original_df[feature].mode().iloc[0] if len(original_df[feature].mode()) > 0 else 0
                else:
                    input_df[feature] = 0
        ...
        for col in input_df.columns:
            if input_df[col].dtype == 'object':
                ...
                if col in original_df.columns:
                    # Fit on original data and transform input
        ...
        return {
            ...
            'probability_superhost': float(probability[1]),
        ...
        return {
            'prediction': 0,
            'probability_regular': 0.5,
            'probability_superhost': 0.5,
            'confidence': 0.5,
            'error': str(e)
        }
...
def display_prediction_result(result, input_data):
    ...
    with col1:
        if result['prediction'] == 1:
            st.success("🌟 **SUPERHOST PREDICTION**")
            st.metric("Superhost Probability", f"{result['probability_superhost']:.1%}")
        else:
            st.info("👤 **REGULAR HOST PREDICTION**")
            st.metric("Regular Host Probability", f"{result['probability_regular']:.1%}")
    ...
    prob_df = pd.DataFrame({
        ...
        'Probability': [result['probability_regular'], result['probability_superhost']]
    })
    ...
    # Create factors dataframe for visualization
    factors = {
        'Review Scores': {
            'Cleanliness': input_data.get('review_scores_cleanliness', 0),
            'Location': input_data.get('review_scores_location', 0),
            'Communication': input_data.get('review_scores_communication', 0),
            'Check-in': input_data.get('review_scores_checkin', 0),
            'Accuracy': input_data.get('review_scores_accuracy', 0)
        },
        'Host Behavior': {
            'Response Time': input_data.get('host_response_time', ''),
            'Identity Verified': '✅ Yes' if input_data.get('host_identity_verified', 0) else '❌ No',
            'Phone Verification Required': '✅ Yes' if input_data.get('require_guest_phone_verification', 0) else '❌ No'
        },
        'Property Details': {
            'Property Type': input_data.get('property_type', ''),
            'Accommodates': input_data.get('accommodates', 0),
            'Bedrooms': input_data.get('bedrooms', 0),
            'Bathrooms': input_data.get('bathrooms', 0),
            'Price': f"${input_data.get('price', 0):.0f}",
            'Amenities': input_data.get('amenities_count', 0)
        }
    }
    ...
    with col1:
        ...
        for key, value in factors['Review Scores'].items():
            if isinstance(value, (int, float)) and value > 0:
                # Color coding for review scores
                if value >= 4.5:
                    st.success(f"{key}: {value:.1f} ⭐")
                elif value >= 4.0:
                    st.info(f"{key}: {value:.1f} ⭐")
                elif value >= 3.5:
                    st.warning(f"{key}: {value:.1f} ⭐")
                else:
                    st.error(f"{key}: {value:.1f} ⭐")
    
    with col2:
        st.markdown("**👤 Host Behavior:**")
        for key, value in factors['Host Behavior'].items():
            st.write(f"**{key}:** {value}")
    
    with col3:
        st.markdown("**🏠 Property Details:**")
        for key, value in factors['Property Details'].items():
            st.write(f"**{key}:** {value}")
    ...
    # Review score recommendations
    avg_review = np.mean([v for v in factors['Review Scores'].values() if isinstance
